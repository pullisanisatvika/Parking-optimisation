...
VAR_ALPHA_HIGH = 0.80                  # C7
VAR_ALPHA_LOW = 0.20                   # C8
VAR_CMAX = 15.0                        # C9
...
def vary_parking_cost_only(base_case):
    ...
    rng = random.Random(
        f"parking-cost-high-{case['number_of_nodes']}-{case['instance_id']}"
    )
    ...
    # Ensure the high-cost scenario remains solvable by keeping at least a small
    # set of walking-feasible parking lots affordable under Cmax.
    affordable_feasible = [
        p for p in case["feasible_parking_nodes"]
        if case["parking_costs"][p] <= case["cmax"]
    ]

    min_affordable_feasible = min(2, len(case["feasible_parking_nodes"]))
    if len(affordable_feasible) < min_affordable_feasible:
        ...
        upper = min(case["cmax"], HIGH_PARKING_COST_RANGE[1])
            ...
            if lower <= upper:
                ...
                for slot in candidate_slots:
                    ...
                    if proposed not in used_costs:
                        ...
                        break
                case["parking_costs"][p] = replacement if replacement is not None else round(rng.uniform(lower, upper), 2)
            else:
