...
MEDIUM_WALK_VALUES = WALK_VALUES
MEDIUM_SUPPLY_VALUES = SUPPLY_VALUES
MEDIUM_BUDGET_VALUES = BUDGET_VALUES
MEDIUM_COST_RANGES = COST_RANGES
TRAFFIC_VALUES = [
    ("very_low", (60.0, 80.0)),
...
MASTER_RESULTS_BY_METHOD = {
    "heuristics": ROOT / "HEURISTICS" / "data" / "master_results_heuristic.csv",
...
def _write_sumo_parking_subset(source_dir: Path, target_dir: Path, parking_pct: float, total_nodes: int) -> Path:
    with (source_dir / "parking_nodes.csv").open(newline="", encoding="utf-8") as handle:
        parking_rows = list(csv.DictReader(handle))
        parking_fields = list(csv.DictReader((source_dir / "parking_nodes.csv").open(newline="", encoding="utf-8")).fieldnames or [])
    with (source_dir / "walking_edges.csv").open(newline="", encoding="utf-8") as handle:
        walking_rows = list(csv.DictReader(handle))
        walking_fields = list(csv.DictReader((source_dir / "walking_edges.csv").open(newline="", encoding="utf-8")).fieldnames or [])
...
def generate_synthetic_parameter_cases(node_count: int, scale_category: str, out_dir: Path) -> list[dict]:
    ...
    manifests: list[dict] = []
    max_supply_case = _supply_variant(base, max(supply_values := (MEDIUM_SUPPLY_VALUES if scale_category == "medium_large" else SUPPLY_VALUES)))
    budget_base = _budget_family_base_case(base)
    walk_values = MEDIUM_WALK_VALUES if scale_category == "medium_large" else WALK_VALUES
    supply_values = list(supply_values)
    budget_values = MEDIUM_BUDGET_VALUES if scale_category == "medium_large" else BUDGET_VALUES
    cost_ranges = MEDIUM_COST_RANGES if scale_category == "medium_large" else COST_RANGES

    for walk in walk_values:
        case = _walking_variant(base, walk)
        case = _metadata(case, scale_category, "maximum_walking_distance", f"{walk:.2f}", fd.BASE_PARKING_PCT)
    ...
    for pct in supply_values:
        active_value = f"{int(round(pct * 100))}%"
        case = _subset_supply_case(base, max_supply_case, pct)
        case = _metadata(case, scale_category, "parking_supply", active_value, pct)
    ...
    for cmax in budget_values:
        case = _budget_variant(budget_base, cmax)
        case = _metadata(case, scale_category, "parking_budget_cmax", f"{cmax:.1f}", fd.BASE_PARKING_PCT)
    ...
    for range_label, cost_range, mode_label in cost_ranges:
        ...
        case = _cost_variant(base, range_label, cost_range, mode_label)
        case = _metadata(case, scale_category, "parking_cost_variation_mode", range_label, fd.BASE_PARKING_PCT)
    ...
    for traffic_label, speed_range in TRAFFIC_VALUES:
        case = _traffic_variant(base, speed_range, traffic_label)
        case = _metadata(case, scale_category, "traffic_level", traffic_label, fd.BASE_PARKING_PCT)
...
def generate_edge_density_only() -> None:
    rows: list[dict] = []
    for node_count in SMALL_NODES:
        rows.extend(generate_edge_density_cases(node_count, "small", SMALL_DIR))
    for node_count in MEDIUM_LARGE_NODES:
        rows.extend(generate_edge_density_cases(node_count, "medium_large", MEDIUM_DIR))
...
def generate_all() -> None:
    rows = []
    for node_count in SMALL_NODES:
        rows.extend(generate_synthetic_parameter_cases(node_count, "small", SMALL_DIR))
    for node_count in MEDIUM_LARGE_NODES:
        rows.extend(generate_synthetic_parameter_cases(node_count, "medium_large", MEDIUM_DIR))
...
def _run_partition(scale_category: str, dat_dir: Path) -> None:
    ...
    target_cases = {path.name for path in dat_paths}
    ...
    for method in METHODS:
        ...
        if cfg["results"].exists():
            ...
            master_results = MASTER_RESULTS_BY_METHOD[method]
            if not master_results.exists():
                raise FileNotFoundError(f"Missing both partition and master results for {method}")
            dlp.normalize_results_csv(method, master_results)
            filtered_rows = [
                row
                for row in _read_csv_rows(master_results)
                if dlp.normalize_case_name(row.get("dat_file", "")) in target_cases
            ]
            cfg["results"].parent.mkdir(parents=True, exist_ok=True)
            with cfg["results"].open("w", newline="", encoding="utf-8") as handle:
                writer = csv.DictWriter(handle, fieldnames=dlp.CANONICAL_RESULTS_HEADER)
                writer.writeheader()
