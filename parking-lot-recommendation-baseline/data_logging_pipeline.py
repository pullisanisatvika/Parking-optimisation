...
def selected_methods(methods: list[str] | None) -> list[str]:
    if not methods:
...
def selected_nodes(nodes: list[int] | None) -> list[int]:
    if not nodes:
        return list(ACTIVE_SYNTHETIC_NODES)
    ordered = [node for node in ACTIVE_SYNTHETIC_NODES if node in nodes]
    if not ordered:
        raise ValueError("No valid node sizes selected.")
    return ordered


def apply_output_suffix(path: Path, suffix: str | None) -> Path:
    if not suffix:
        return path
    return path.with_name(f"{path.stem}_{suffix}{path.suffix}")


def config_with_suffix(cfg: dict, suffix: str | None) -> dict:
    if not suffix:
        return cfg
    out = dict(cfg)
    out["results"] = apply_output_suffix(cfg["results"], suffix)
    out["routes"] = apply_output_suffix(cfg["routes"], suffix)
    return out
...
@lru_cache(maxsize=None)
def _lot_objective_context_for_dat(dat_file: str):
    meta = _parse_dat_metadata(dat_file)
    if meta is None:
        return None
    text = meta["dat_path"].read_text(encoding="utf-8", errors="replace")
    data = parse_lot_dat_for_normalization(text)
    return build_lot_normalization_context(data)
...
def _force_reconcile_row_with_dataset(row: dict, route_row: dict | None = None) -> dict:
    ...
    if (
        ...
        context = _lot_objective_context_for_dat(out.get("dat_file", ""))
        if context is not None:
            out["objective"] = str(normalized_lot_objective(total_time, parking_cost, context=context))
...
def run_final_cases(cmax: float, methods: list[str] | None = None, suffix: str | None = None):
    ...
    method_list = selected_methods(methods)
    for method in method_list:
        cfg = config_with_suffix(method_config(method, "final_cases"), suffix)
        _ensure_clean_outputs(cfg["results"], cfg["routes"])
        for dat_path in dat_paths:
    ...
    if method_list == METHOD_ORDER:
        write_combined_final_cases()
        print(f"Updated combined files in: {COMBINED_DIR}", flush=True)
    else:
        print("Skipped combined final-cases rebuild because only a method subset was run.", flush=True)


def run_sumo_cases(cmax: float, methods: list[str] | None = None, suffix: str | None = None):
    ...
    for method in selected_methods(methods):
        cfg = config_with_suffix(method_config(method, "sumo"), suffix)
        _ensure_clean_outputs(cfg["results"], cfg["routes"])
        for dat_path in dat_paths:
...
def run_all_node_major(
    ...
    all_nodes = selected_nodes(nodes)
    method_list = selected_methods(methods)
    ...
    if method_list == METHOD_ORDER:
        ...
        print("\n--- SKIPPED COMBINED RESULTS (SUBSET METHOD RUN) ---", flush=True)
...
def main():
    ...
    p1.add_argument("--methods", nargs="+", choices=METHOD_ORDER, default=None)
    p1.add_argument("--suffix", default=None)
    ...
    p2.add_argument("--methods", nargs="+", choices=METHOD_ORDER, default=None)
    p2.add_argument("--suffix", default=None)
    ...
    p2s.add_argument("--methods", nargs="+", choices=METHOD_ORDER, default=None)
    p2s.add_argument("--suffix", default=None)
    ...
    p3.add_argument("--methods", nargs="+", choices=METHOD_ORDER, default=None)
    p3.add_argument("--nodes", nargs="+", type=int, default=None)
    p3.add_argument("--suffix", default=None)
    ...
    sub.add_parser("combine-final-cases", help="Rebuild combined final-case files from existing per-method outputs.")
    args = parser.parse_args()
    try:
        if args.cmd == "run-partitioned-c1":
            run_partitioned_c1(args.cmax)
        elif args.cmd == "run-final-cases":
            run_final_cases(args.cmax, args.methods, args.suffix)
        elif args.cmd == "run-sumo-cases":
            run_sumo_cases(args.cmax, args.methods, args.suffix)
        elif args.cmd == "run-all-node-major":
            run_all_node_major(args.cmax, args.methods, args.nodes, args.suffix)
        ...
        elif args.cmd == "combine-final-cases":
            write_combined_final_cases()
            print(f"Updated combined files in: {COMBINED_DIR}", flush=True)
        else:
